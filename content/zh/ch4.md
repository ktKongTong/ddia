...
具体来说，针对事务性工作负载 (OLTP) 优化的存储引擎与针对分析优化的存储引擎之间存在很大巨异（我们在 ["分析型与事务型系统"](/ch1#sec_introduction_analytics) 中介绍了这种区别）。本章首先介绍两种用于 OLTP 的存储引擎：写入不可变数据文件的 *日志结构* 存储引擎，以及以及像 *B 树* 这样就地更新数据的存储引擎。这些结构既可用于键值存储，也可用于二级索引。
...
这两个函数实现了键值存储。你可以调用 `db_set key value`，它将在数据库中存储 `key` 和 `value`。键和值几乎可以是任何你喜欢的内容 —— 例如，值可以是一个 JSON 文档。然后你可以调用 `db_get key`，它会查找并返回与该特定键关联的最新值。
