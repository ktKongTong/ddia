...
从最基本的层面上讲，数据库需要做两件事：当你给它一些数据时，它应该存储这些数据；当你稍后再询问它时，它应该把数据返回给你。

在 [第 3 章](/ch3#ch_datamodels) 中，我们讨论了数据模型和查询语言 —— 即你向数据库提供数据的格式，以及你之后再次请求数据的接口。在本章中，我们从数据库的角度讨论同样的问题：数据库如何存储你提供的数据，以及当你请求时如何再次找到这些数据。

作为应用开发者，你为什么要关心数据库内部如何处理存储和检索？你可能不会从头开始实现自己的存储引擎，但你 *确实* 需要从众多可用的存储引擎中选择一个适合你应用的。为了让存储引擎在你的工作负载类型上表现良好，你需要对存储引擎底层工作原理有个大致的了解。

具体来说，针对事务性工作负载 (OLTP) 优化的存储引擎与针对分析优化的存储引擎之间存在很大巨异（我们在 ["分析型与事务型系统"](/ch1#sec_introduction_analytics) 中介绍了这种区别）。本章首先介绍两种用于 OLTP 的存储引擎：写入不可变数据文件的 *日志结构* 存储引擎，以及像 *B 树* 这样就地更新数据的存储引擎。这些结构既可用于键值存储，也可用于二级索引。
...
这两个函数实现了键值存储。你可以调用 `db_set key value`，它将在数据库中存储 `key` 和 `value`。键和值（几乎）可以是任何你喜欢的内容 —— 例如，值可以是一个 JSON 文档。然后你可以调用 `db_get key`，它会查找并返回与该特定键关联的最新值。
...
存储格式非常简单：一个文本文件，每行包含一个以逗号分隔的键值对（类似于 CSV 文件，忽略转义问题）。每次调用 `db_set` 都会将记录追加到文件末尾。如果多次更新同一个键，旧版本的值不会被覆盖 —— 你需要查看文件中键最后一次出现的位置来找到最新值（因此 `db_get` 中使用了 `tail -n 1`）：
...
对于如此简单的实现，`db_set` 函数实际上有相当好的性能，因为将数据追加到文件通常非常高效。与 `db_set` 所做的类似，许多数据库内部使用一个仅追加的数据文件：*日志*。真正的数据库有更多问题要处理（如处理并发写入、回收磁盘空间以防日志无限增长，以及从崩溃中恢复时处理部分写入的记录），但基本原理是相同的。日志非常有用，我们将在本书中多次遇到它们。
...
另一方面，如果你的数据库中有大量记录，`db_get` 函数的性能会很糟糕。每次查找某个键时，`db_get` 必须从头到尾扫描整个数据库文件，查找该键出现的位置。用算法术语来说，查找的复杂度是 *O*(*n*)：如果数据库中的记录数 *n* 翻倍，查找时间也会翻倍。这并不好。

为了高效地查找数据库中特定键的值，我们需要一种不同的数据结构：*索引*。本章将介绍各种索引结构，并比较它们的区别；索引的一般思路是将数据以特定的方式（例如，按某个键排序）组织起来，以便更快地找到所需的数据。如果你想以多种不同的方式搜索相同的数据，则可能需要针对数据的不同部分建立多个不同的索引。

索引是从主数据派生出来的*附加*结构。许多数据库允许你添加和删除索引，这不会影响数据库的内容；它只会影响查询的性能。维护额外的结构会产生开销，尤其是在进行写入时。对于写入操作而言，很难找到比“简单地将数据追加到文件”更优的性能方案，因为这是最简单的写入操作。任何类型的索引通常都会减慢写入速度，因为每次写入数据时都需要更新索引。

这是存储系统中的一个重要权衡：精心选择的索引加快了读查询速度，但每个索引都会消耗额外的磁盘空间并减慢写入速度，有时会大幅减慢 [^1]。因此，数据库通常不会默认为所有内容建立索引，而是要求你 —— 编写应用程序或管理数据库的人 —— 根据你对应用典型查询模式的了解来手动选择索引。这样，你就可以选择能够为应用带来最大收益的索引，而不会增加不必要的写入开销。
...
* 哈希映射不是持久化的，所以当你重启数据库时必须重建它 —— 例如，通过扫描整个日志文件来找到每个键的最新字节偏移量。如果数据量很大，这会使重启变慢。
* 哈希表必须适合内存。原则上，你可以在磁盘上维护哈希表，但不幸的是，很难让磁盘上的哈希映射表现良好。它需要大量的随机访问 I/O，当它变满时扩展成本高昂，哈希冲突需要复杂的逻辑 [^2]。
* 范围查询效率不高。例如，你不能轻松扫描 `10000` 和 `19999` 之间的所有键 —— 你必须在哈希映射中逐个查找每个键。

#### SSTable 文件格式 {#the-sstable-file-format}

在实际应用中，哈希表并不常被用作数据库索引。相反，更常见的做法是将数据保存在一种 *按键排序* 的结构中 [^3]。这种结构的一个典型例子是 *排序字符串表*（*Sorted String Table*，简称 SSTable），如 [图 4-2](#fig_storage_sstable_index) 所示。这种文件格式同样存储键值对，但它保证所有数据按键顺序排列，并且每个键在文件中只出现一次。

{{< figure src="/fig/ddia_0402.png" id="fig_storage_sstable_index" caption="图 4-2. 具有稀疏索引的 SSTable，可以查询跳转到正确的块。" class="w-full my-4" >}}

现在你不需要在内存中保留所有键：你可以将 SSTable 中的键值对分组为几千字节的 *块*，然后将每个块的第一个键存储在索引中。这种只存储部分键的索引称为 *稀疏索引*。这个索引通常存储在 SSTable 的独立部分中，可以使用不可变的 B 树、字典树或其他能够高效查询特定键的数据结构来实现 [^4]。

例如，在 [图 4-2](#fig_storage_sstable_index) 中，一个块的第一个键是 `handbag`，下一个块的第一个键是 `handsome`。假设现在你要查找键 `handiwork`，但它并未出现在稀疏索引中。由于键是有序的，你知道 `handiwork`  一定位于 `handbag` 和 `handsome` 之间。这意味着你可以根据索引定位到 `handbag` 的偏移量，然后从那里开始顺序扫描文件，直到找到 `handiwork`（或没有，如果该键不在文件中）。每个块的大小只有几千字节，因此扫描速度非常快。
